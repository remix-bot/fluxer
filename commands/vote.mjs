...
export const command = new CommandBuilder()
  ...
  .addStringOption(o =>
    o.setName("id")
      .setDescription("The server or bot UUID (uses config default if not provided)")
      .setRequired(false)
  )
  .addNumberOption(o =>
    o.setName("page")
...
function noResourceId(type, prefix) {
  return {
    embeds: [new EmbedBuilder()
      .setColor("#ff0000")
      .setDescription(`No ${type} ID configured. Provide it with \`${prefix}vote check ${type} <uuid>\` or set it in config.json.`)]
  };
}
