...
export class GatewayHandler {
    ...
    for (let i = 0; i < channels.length; i++) {
      ...
      setTimeout(async () => {
        try {
          ...
          if (!ch) {
            logger.voice247(`[BootRecovery] Channel ${channelId} no longer exists — removing 24/7 state`);
          ...
          const msg = e?.message ?? "";
          // 401 Unauthorized means the bot lost access to the channel (kicked,
          // lost permissions, or channel deleted). Auto-remove the 24/7 state
          // so we don't keep retrying on every reboot.
          if (msg.includes("401") || msg.includes("Unauthorized") ||
              msg.includes("NOT_LOGGED_IN") || msg.includes("Forbidden")) {
            logger.warn(`[BootRecovery] 401 for channel ${channelId} — bot lacks access, removing 24/7 state`);
            this._remove247ForChannel(guildId, channelId);
          } else {
            logger.warn(`[BootRecovery] Failed to rejoin 24/7 channel ${channelId}: ${msg}`);
          }
        }
      }, delay);
    }
  ...
  /**
   * Remove 24/7 state for a specific channel when the bot can't access it.
   * This prevents repeated 401 failures on every reboot.
   *
   * @param {string} guildId
  ...
  _remove247ForChannel(guildId, channelId) {
    ...
    const channels = Array.isArray(raw)
      ? new Set(raw.map(id => String(id).replace(/\D/g, "")))
      : new Set([String(raw).replace(/\D/g, "")]);
    const cleanCh = String(channelId).replace(/\D/g, "");

    if (!channels.has(cleanCh)) return;

    channels.delete(cleanCh);
    // Use the shared helper to remove the per-channel mode entry and update the guild-wide mode
    remove247ChannelMode(set, cleanCh, channels);
    set.set("stay_247", channels.size > 0 ? [...channels] : "none");

    logger.voice247(`[BootRecovery] Removed 24/7 for channel ${cleanCh} in guild ${guildId} (inaccessible)`);
  }
