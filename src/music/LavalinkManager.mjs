/** @module src/music/LavalinkManager @description Manages Lavalink/NodeLink connection, voice forwarding, and audio search functionality. */
...
/** Create a new LavalinkManager and initialize the underlying lavalink-client connection. @param {object} nodeCfg - Node configuration (id, host, port, password, secure). @param {object} client - bot client instance. @param {object} user - Bot user object with id and username. */
  ...
  _sendPayload(guildId, payload) {
    try {
      ...
      if (typeof this._client?.ws?.send === "function") {
        this._client.ws.send(targetShardId, payload);
        return;
      }

      const shard = ShardingUtils.getShard(this._client, targetShardId);
      if (!shard) {
        ...
        for (const old of attached) {
          ...
          try {
            if (typeof old.removeEventListener === "function") {
              ...
              old.removeEventListener("error", this._rawWsErrorHandler);
            } else if (typeof old.off === "function") {
              old.off(
                "message",
                this._rawWsMessageListener ?? this._rawWsHandler
              );
              old.off("error", this._rawWsErrorHandler);
            }
          } catch (e) {
            logger.warn(
